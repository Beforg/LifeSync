...
public class ApplicationController implements Initializable {
    ...
    @FXML
    private Button btCriaTarefaHome;
    ...
    private Label homeNomeTarefa,homeCategoriaTarefa,homePrioridadeTarefa,homeTipoTreino,homeStatusTreino
            ,homeHabitosNome,homeStatusHabito,homeStatusTarefa,progressoHomeLabel,tituloSemana,guardaIdConta,labelRegistraTreino;
    @FXML
    ...
    private Pane paneHome, panePerfil, paneTarefas, paneEstudos, paneAtividades,cadastro, login, containerAplicacao,
            ...
            paneEstatisticasEdados,paneAddTarefaHome;
    @FXML
    private Label muralSemanaDia,muralCalendarioMesAtual,muralCalendarioAtrasadas,muralCalendarioDia, labelDateTime,
            ...
            muralDescricaoTarefa, muralCalendarioTarefasMes,muralCalendarioConcluidas,labelAvisosTarefas,labelAvisosTarefasMural,labelAvisaTarefaHome;
    @FXML
    ...
    private Label muralSemanaTarefasSemana, muralSemanaConcluidas,muralSemanaAtrasadas,selecionadaTarefaConcluida,
            selecionadaTarefaNumero,labelCalendarioAtrasadas,treinosFiltrados1;
    @FXML
    ...
    @FXML
    private TextField textDescricao,tfNomeCadastro, tfNomeUsuarioCadastro, tfUserLogin,tfNomeTarefa,tfNomeCategoria, editarNomeTarefa,tfNomeTarefaHome,tfDescricaoHome ;
    @FXML
    ...
    private ChoiceBox<String> cbCategoriaTarefa, cbPrioridadeTarefa, editarPrioridadeTarefa, editarCategoriaTarefa,
            filtroTarefa,filtroTarefaSub,choiceBoxEscolhaSubmeta,cbCategoriaHome,cbPrioridadeHome,cbSubmetaHome;
    @FXML
    ...
    @FXML
    private TextArea  textAreaEditarTarefa;
    @FXML
    ...
    public void tarefaView(){
        if (adicionarTarefas.isVisible()) {
            adicionarTarefas.setVisible(false);
            if (calendarioFull.isVisible()) {
                muralCalendario.setVisible(true);
            } else if (tabelaTarefas.isVisible()) {
                paneMuralTarefas.setVisible(true);
            } else if (gridSemana.isVisible()) {
                muralCalendario.setVisible(true);
                gridSemana.setVisible(false);
                calendarioFull.setVisible(true);
            }
        } else {
            exibirAdicionarTarefa(true);
        }

    }

    private void exibirAdicionarTarefa(boolean visivel) {
        gridTarefaSelecionadaSemana.setVisible(visivel);
        gridNovaTarefa.setVisible(visivel);
        gridCategoria.setVisible(visivel);
        gridTarefaSelecionadaCalendario.setVisible(!visivel);
        gridDetalhesDaTabelaTarefa.setVisible(!visivel);
        muralCalendario.setVisible(!visivel);
        paneMuralTarefas.setVisible(!visivel);
        adicionarTarefas.setVisible(visivel);
    }

    @FXML
    void mostrarAddTarefaHome() {
        if (paneAddTarefaHome.isVisible()) {
            paneAddTarefaHome.setVisible(false);
        } else {
            paneAddTarefaHome.setVisible(true);
        }
    }
    ...
    public void salvarCategoria(){
        if (tfNomeCategoria.getText().isEmpty()) {
            labelAvisosTarefas.setText("Nome da categoria não pode ser vazio");
    ...
    public void registrarManual() {
        ValidacaoDados.validaRegistroEstudo(tfNomeManual,datePickerManual, labelAvisosEstudos); // add alert de erro
    ...
    public void registrarCronometrado() {
        if (labelCronometro.getText().equals("00:00:00") || tfNomeCronometrado.getText().isEmpty()) {
            ...
            if (resposta == 1) {
                ...
                NotificacaoUtil.tocarSom("/auidio/pomodoro.mp3");
                NotificacaoUtil.mostrarNotificacao("Estudo finalizado", "Estudo finalizado com sucesso", "Estudo finalizado com sucesso");
                tfNomeCronometrado.setText("");
                textAreaDescricaoCronometrado.setText("");
                btConfigPomodoro.setDisable(false);
    ...
    private Label labelAvisosNovaMeta,labelCriarArea,mmNomeMeta, nmDataMeta,nmAreaMeta,nmStatus,nmAssociadasMeta,
            ...
            sobreTarefasFeitasSubmetas,labelAvisosSubmeta,labelSubmetasMetas,labelTarefasMetas;
    @FXML
    ...
    @FXML
    private Button btCriarArea,btCriarMeta,btApagarMeta,btSalvarMeta,btSalvarEdicaoSubmeta,btCards;
    @FXML
    ...
    @FXML
    private TextField taAddTarefaSubmeta;
    @FXML
...
//refatoraar
    ...
    public void minhasMetasView() {
        ...
        labelSubmetasMetas.setVisible(true);
        labelTarefasMetas.setVisible(true);
    ...
    public void submetaView() {
        ...
        paneInferiorNovaMeta.setVisible(false);
        labelSubmetasMetas.setVisible(false);
        labelTarefasMetas.setVisible(false);
    ...
    public void criarTarefaEmMeta() {
        ...
        CarregaDadosService.carregaDadosEtarefasAssociadasDaSubmeta(sobreNomeSubmeta,sobreDataSubmeta,
                ...
                tarefaRepository,btApagaTreino,btEditarSubmeta);
    }
    ...
    @FXML
    private Pane paneOpcaoProjetos,paneInferiorProjetos,paneOpcaoTreino,paneInferiorTreino, paneProjetos, paneCards,paneAddTreino;
    @FXML
    ...
    public void novoTreinoView() {
       //TreinoUtil.trocaTelasControle(gridPaneInicialTreinos,gridPaneAtividadesNovoTreino,gridPaneAtividadesMeusTreinos,true,false,false);
        gridPaneAtividadesNovoTreino.setVisible(true);
        paneAddTreino.setVisible(false);
    ...
    public void meusTreinosView() {
        ...
        gridPaneAtividadesNovoTreino.setVisible(false);
        paneAddTreino.setVisible(true);
    ...
    public void criarTreino() {
        ...
        TreinoService.criarTreino(labelAvisosTreino,tfNomeTreino,dpDataTreino,listaMeusTreinos,cbTipoTreino,
                taDescricaoTreino,tfNovoTipoTreino,treinoRepository, rbNovoTipo,btVoltarEditarTreino,guardaNomeTreino,paneHome);
    ...
    public void editarTreinoView() {
        TreinoUtil.guardaNomeDoTreinoParaEditar(guardaNomeTreino,tabelaTreinos);
        ...
        paneAddTreino.setVisible(false);
        gridPaneAtividadesNovoTreino.setVisible(true);
        CarregaDadosService.carregaDadosParaEdicaoDoTreino(tabelaTreinos,dpDataTreino,cbTipoTreino,
                taDescricaoTreino,tfNomeTreino,treinoRepository,false);
        atualizaDadosHome();
        atualizarTabelaDeTreino();
    }
    public void apagarTreino() {
        TreinoService.apagarTreino(tabelaTreinos,treinoRepository);
        atualizarTabelaDeTreino();
        atualizaDadosHome();
        CarregaDadosService.atualizaListaDeTreinos(listaMeusTreinos,treinoRepository);
    }
    ...
    public void atualizarTabelaDeTreino() {
        CarregaDadosService.carregarDadosParaTabelaDeTreino(guardaNomeTreino,tfDuracaoTreino,checkBoxSemTempo,
                btRegistrarTreino,btVoltarRegistroCompleto,labelRegistraTreino,qtdTreinosMeusTreinos,
    ...
    public void voltarFiltroView() {
        TreinoUtil.exibeRegistroCompletoDoTreino(tfDuracaoTreino,checkBoxSemTempo,btRegistrarTreino,btVoltarRegistroCompleto,labelRegistraTreino,false);
    }
    ...
    public void projetosView() {
        paneInferiorProjetos.setVisible(true);
        gridPaneMuralProjetos.setVisible(true);
        ...
        ProjetoUtil.controleDeAtividades(paneOpcaoProjetos,new Pane(),paneOpcaoTreino,paneInferiorTreino,tabelaTreinos,true,false);
        atividades();
    }
    public void meusProjetosView() {
        paneInferiorProjetos.setVisible(false);
        ...
        AppUtils.viewMenuProjetos(paneProjetos,paneCards,true);
    }
    public void editarProjetoView() {
        gridPaneEditaProjeto.setVisible(true);
        gridPaneNovoProjeto.setVisible(false);
    ...
    public void novoProjetoView() {
        gridPaneNovoProjeto.setVisible(true);
    ...
    public void novoCardView() {
        gridPaneNovoCard.setVisible(true);
    ...
    public void editarCard() {
        gridPaneEditaCard.setVisible(true);
        gridPaneNovoCard.setVisible(false);
    ...
    public void criarCard() {
        gridPaneNovoCard.setVisible(false);
        ProjetoCardService.criarCardEAdicionarAoVBox(tfNomeCard,taCriarCard,
                cbEscolheProjetoCard,dpCriarCard,labelAvisosProjetos,
                projetoCardRepository,projetoRepository,vBoxNaoIniciada, nomeDoCard,descricaoCard,cbStatusCard,tabelaItensCard,itemProjetoCardRepository,tcNomeItemCard);
        atualizaProjetos();
    }
    private void atualizaProjetos() {
        ...
        CarregaDadosService.carregaMeusCards(cbCardsRelacionadosMeusProjetos,projetoCardRepository,cbMeusProjetos.getValue());
        //cbCardsRelacionadosMeusProjetos
    }
    ...
    public void addItemView() {
       // ProjetoUtil.novoProjetoEcardView(gridPaneCriaItem,gridPaneNovoProjeto,gridPaneNovoCard,gridPaneMuralProjetos,gridPaneMeusProjetos,gridPaneEditaCard,gridPaneEditaProjeto,false,false,true);
        ProjetoUtil.carregaNomeDoCard(nomeDoCard,nomeDoCardItem,tcNomeItemCard);
        gridPaneCriaItem.setVisible(true);
        tabelaItensCard.setVisible(false);
    }
    public void criarItem() {
        gridPaneCriaItem.setVisible(false);
        tabelaItensCard.setVisible(true);
        ...
        ItemProjetoCardService.carregarItemNoCard(tabelaItensCard,itemProjetoCardRepository,nomeDoCard);
    }
    @FXML
    void mostrarMeusCards() {
        AppUtils.viewMenuProjetos(paneProjetos,paneCards,false);
    }
    ...
    @FXML
    private RadioButton rbCriarNovoHabito, rbRepete;
    @FXML
    private DatePicker dpDiaHabito,dpDiaHabitoRepeat;
    @FXML
    ...
    @FXML
    private Button btPassarMesHabito,btAtualHabitos,btVoltarHabitos,btRemoverHabito,btApagarHabito, btVincularHabito;
    @FXML
    ...
    @FXML
    private Label labelDiaHabitoAdd, labelRepeteHabito,labelHabitoNomeAdd;
    @Autowired
    private HabitosRepository habitosRepository;

    @FXML
    void addHabitoView() {
        AppUtils.mostraAdicionarHabito(labelDiaHabitoAdd,labelRepeteHabito,labelHabitoNomeAdd,rbCriarNovoHabito,rbRepete,btVincularHabito,btApagarHabito,dpDiaHabito,dpDiaHabitoRepeat,
                cbSelecionarHabito, !labelHabitoNomeAdd.isVisible());
    }
    public void criarHabito() {
        HabitosService.criarHabito(labelAvisosHabitos,
                dpDiaHabito,tfCriarHabito,cbSelecionarHabito,categoriaRepository,habitosRepository,rbCriarNovoHabito,rbRepete,dpDiaHabitoRepeat);
    ...
    public void checkConcluirHabito() {
        if (checkBoxHabitos.isSelected()) {
            checkBoxHabitos.setText("Concluído");
        } else {
            checkBoxHabitos.setText("Concluir");
        }
    ...
    @FXML
    private CheckBox cbRepeteCusto;
    @FXML
    private ChoiceBox<String> cbCategoriaFinanca;
    @FXML
    private Label labelSaldo,labelRenda,labelGastos,labelAvisosFinancas,labelCategoria,labelDescricao,labelValor,labelData;
    @FXML
    ...
    @FXML
    private DatePicker dpSelecionaMesAtual,dpDataDoRegistro,dpRepeteCustoFinanca;
    ...
    public void registraGasto() {
        ...
        FinancaUtil.exibeRegistrarCusto(labelCategoria,cbCategoriaFinanca,
                rbNovaCategoriaFinanca,labelDescricao,tfDescricaoFinanca,labelValor,tfValorGasto,labelData,dpDataDoRegistro,cbRepeteCusto,dpRepeteCustoFinanca,btSalvarGasto,false);
    }
    @FXML
    void registraCustoView() {
        FinancaUtil.exibeRegistrarCusto(labelCategoria,cbCategoriaFinanca,
                rbNovaCategoriaFinanca,labelDescricao,tfDescricaoFinanca,labelValor,tfValorGasto,labelData,dpDataDoRegistro,cbRepeteCusto,dpRepeteCustoFinanca,btSalvarGasto,true);
    }
    ...
    @FXML
    private Pane paneAddTreinoHome;
    @FXML
    private Label labelAvisosTreinoHome,labelDescricaoTreinoHome,labelDescricaoTarefaHome,labelProgressoProjeto;
    @FXML
    private Button btCriarTreinoHome,btRemoverHabitoDia,btConcluidoHabitoDia,btAddAoDia,btEditarTreino,btApagaTreino,btEditarSubmeta;
    @FXML
    private TextField tfNomeTreinoHome,tfDescricaoTreinoHome;
    @FXML
    private ChoiceBox<String> cbTipoTreinoHome,cbHabitosHome;
    @FXML
    private ProgressBar pgProjetos;

    @FXML
    void mostrarAddTreinoHome() {
        if (paneAddTreinoHome.isVisible()) {
            paneAddTreinoHome.setVisible(false);
        } else {
            paneAddTreinoHome.setVisible(true);
        }
    }

    private void carregaDadosHome() {
        cbHabitosHome.getItems().clear();
        CarregaDadosService.carregaCategorias(cbCategoriaHome,categoriaRepository);
        CarregaDadosService.carregaSubmetasTarefas(cbSubmetaHome,submetaRepository);
        CarregaDadosService.carregaTiposDeTreino(cbTipoTreinoHome,categoriaRepository);
        CarregaDadosService.carregarHabitos(categoriaRepository,cbHabitosHome);
    }
    @FXML
    void removerHabitoDoDiaHome() {
        HabitosService.removerHabito(new Label(""),habitosRepository,homeHabitosLista,new Label("Home"));
        atualizarHabitosListados();
        atualizaCalendarioHabitos();
        atualizaDadosHome();
        CarregaDadosService.carregaListaDeHabitos(homeHabitosLista,habitosRepository,new Label("Home"));
    }
    @FXML
    void addHabitoHome() {
        HabitosService.criarHabito(new Label(""),
                new DatePicker(LocalDate.now()),tfCriarHabito,cbHabitosHome,categoriaRepository,habitosRepository,new RadioButton(),rbRepete,dpDiaHabitoRepeat); // Verificar depois
        atualizaCalendarioHabitos();
        atualizaDadosHome();
        atualizarHabitosListados();
    }
    @FXML
    void concluirHabitoDiaHome() {
        HabitosService.concluirHabitoHome(homeHabitosLista.getSelectionModel().getSelectedItem(),habitosRepository);
        atualizarHabitosListados();
        atualizaCalendarioHabitos();
        atualizaDadosHome();
        CarregaDadosService.carregaListaDeHabitos(homeHabitosLista,habitosRepository,new Label("Home"));
    ...
    @FXML
    void criarTreinoHome() {
        paneAddTreinoHome.setVisible(false);
        TreinoService.criarTreino(labelAvisosTreinoHome,tfNomeTreinoHome,new DatePicker(LocalDate.now()),listaMeusTreinos,cbTipoTreinoHome,
                new TextArea(tfDescricaoTreinoHome.getText()),tfNovoTipoTreino,treinoRepository, new RadioButton(),btVoltarEditarTreino,guardaNomeTreino,paneHome);
        atualizarTabelaDeTreino();
        atualizaDadosHome();
        carregaDadosHome();
        CarregaDadosService.atualizaListaDeTreinos(listaMeusTreinos,treinoRepository);
    ...
    @FXML
    void criarTarefaHome() {
        TarefaService.salvarTarefa(tarefaRepository,tfNomeTarefaHome,tfDescricaoHome,new DatePicker(),cbPrioridadeHome,
                cbCategoriaHome,conta,cbSubmetaHome,submetaRepository,labelAvisaTarefaHome);
        atualizaTarefa();
        atualizaDadosHome();
        paneAddTarefaHome.setVisible(false);
        carregaDadosHome();
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        CarregaDadosService.carregaDadosEtarefasAssociadasDaSubmeta(sobreNomeSubmeta,sobreDataSubmeta,
                ...
                submetaRepository,tarefaRepository,btApagaTreino,btEditarSubmeta);
        ...
        Filtros.setPrioridade(cbPrioridadeHome);
        ...
        ListenerTarefas.listenerBotoesTarefa(editarTarefaBtExcluir,botaoEditarTarefa,checkBoxConcluido,tabelaTarefas);
        ...
        ListenerProjetos.listenerCardStatus(cbStatusCard,projetoCardRepository,cbSelecionaProjetoMural,vBoxNaoIniciada,
                ...
                naoIniciadasCount, emAndamentoCount,completoCount,diasRestantesProjetos,projetoRepository,tcNomeItemCard,pgProjetos,labelProgressoProjeto);
        ListenerProjetos.listenerProjetoSelecionado(cbSelecionaProjetoMural,vBoxNaoIniciada,vBoxEmAndamento,
                ...
                diasRestantesProjetos,projetoRepository,tcNomeItemCard,pgProjetos,labelProgressoProjeto);
        ListenerHabitos.listenerRadioButton(rbCriarNovoHabito,tfCriarHabito,cbSelecionarHabito);
        ListenerHabitos.listenerRemoverHabito(cbSelecionarHabito,btApagarHabito,btVincularHabito);
        ListenerHabitos.listenerHabitoHome(homeHabitosLista,homeHabitosNome,homeStatusHabito,btConcluidoHabitoDia,btRemoverHabitoDia,habitosRepository);
        ...
        ListenerHome.listenerTarefas(homeTarefasLista,homeNomeTarefa,homeCategoriaTarefa,homePrioridadeTarefa,homeStatusTarefa,labelDescricaoTarefaHome,tarefaRepository);
        ListenerHome.listenerTreinos(homeTreinosLista,homeTipoTreino,homeStatusTreino,labelDescricaoTreinoHome,treinoRepository);
       ...
       // TarefasUtil.gifAtrasado(labelCalendarioAtrasadas,imageViewAtrasadas);
        ...
        AppUtils.listenerHabito(rbCriarNovoHabito,rbRepete,dpDiaHabito,dpDiaHabitoRepeat,cbSelecionarHabito,tfCriarHabito);
        atualizaPomodoro();
        AppUtils.line(paneInferiorProjetos,lineEstudos);
        AppUtils.liberaBotaoCriarTarefaHome(btCriaTarefaHome,tfNomeTarefaHome,cbCategoriaTarefa,cbPrioridadeTarefa);
        AppUtils.listenerAddHabitoDiaHome(cbHabitosHome,btAddAoDia);
        AppUtils.liberaBotaoCriarTreinoHome(tfNomeTreinoHome,cbTipoTreinoHome,btCriarTreinoHome);
        ProjetoUtil.liberaBotaoCard(btCards, cbMeusProjetos);
        ProjetoUtil.liberaChoiceBox(cbStatusCard,nomeDoCard);
        TreinoUtil.liberaBotoesTreino(tabelaTreinos,btEditarTreino,btApagaTreino);
        cbSelecionarHabito.setVisible(false);
        homeSelecionarDiaProgresso.setValue(LocalDate.now());
        ListenerEstudos.liberaBotaoMaterias(btEditarMateria,btExcluirMateria,chpiceBoxMinhasMaterias);
    }
