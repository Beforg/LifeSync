...
public interface HabitosRepository extends JpaRepository<Habitos, Long> {
    ...
    @Query("SELECT h FROM Habitos h WHERE h.nome = :nome AND h.data BETWEEN :dataInicio AND :dataFim")
    List<Habitos> findByIntervaloDeData(String nome, LocalDate dataInicio, LocalDate dataFim);
}
