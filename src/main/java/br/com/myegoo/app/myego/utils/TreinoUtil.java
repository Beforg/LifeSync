...
public class TreinoUtil implements IPadraoDaData, ITabela {
    ...
    public static void guardaNomeDoTreinoParaEditar(Label guardaNomeTreino, TableView<TabelaTreino> listaTreinos) {
        guardaNomeTreino.setText(String.valueOf(listaTreinos.getSelectionModel().getSelectedItem()));
    }
    public static void liberaBotoesTreino(TableView<TabelaTreino> tabelaTreino, Button add, Button apagar) {
        tabelaTreino.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue != null) {
                add.setDisable(false);
                apagar.setDisable(false);
            } else {
                add.setDisable(true);
